...
- プロダクト名: `$ARGUMENTS`
  ...
  - 注意: `/add-eol-product` で使った SBOM の service 名 (例: `rocky-apache-http-server`) とは別物。endoflife.date が認識する名前を使う。
...
```bash
docker exec threatconnectome-db-1 psql -U postgres -d postgres -t -A -F'|' -c \
...
- レポート出力先 `eol-work/<product>-report.md` は `.gitignore` 対象外。コミットしないようユーザーに通知すること (既に `/add-eol-product` で通知済みなら省略可)。
