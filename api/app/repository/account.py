...
class AccountRepository:
    ...
    def get_all(self) -> List[models.Account]:
        return self.db.query(models.Account).all()

    def get_by_firebase_uid(self, account_id: str) -> models.Account | None:
        return self.db.query(models.Account).filter(models.Account.uid == account_id).one_or_none()

    def get_by_id(self, user_id: str) -> models.Account | None:
        return self.db.query(models.Account).filter(models.Account.user_id == user_id).one_or_none()

    def get_by_email(self, account_email: str) -> models.Account | None:
        return (
    ...
    def get_by_team(self, team_id) -> List[models.Account]:
    ...
    def delete(self, account_id) -> models.Account | None:
        account_to_delete = (
            self.db.query(models.Account).filter(models.Account.id == account_id).first()
        )
