...
def upgrade() -> None:
    ...
    like_patterns = [f"'{prefix}%'" for prefix in OS_TYPES]
    array_literal = ",".join(like_patterns)
    ...
    connection.execute(
        ...
        WITH package_types AS (
            SELECT 
                ...
                ecosystem,
                CASE
                    WHEN lower(ecosystem) LIKE ANY(ARRAY[{array_literal}]) THEN 'OS'
                    ELSE 'LANG'
...
def downgrade() -> None:
    ...
    connection.execute(
        ...
        WHERE package_version_id IN (
            ...
            JOIN package p ON pv.package_id = p.package_id
            WHERE p.type = 'OS'
            AND p.source_name IS NOT NULL
            AND p.source_name <> p.name
            AND p.source_name IN (
                SELECT a.affected_name FROM affect a
            )
        )
        """
        )
