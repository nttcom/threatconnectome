...
def create_deployer_dict() -> None:
    try:
        ...
        with open(deployer_file_path, "r") as deployer_file:
            ...
            create_human_impact_dict(deployer_json)
            create_ssvc_priority_dict(deployer_json)
    except KeyError:
        pass


def create_human_impact_dict(deployer_json) -> None:
    human_impact_dict.clear()
    for decision_point in deployer_json["decision_points"]:
        if decision_point["label"] == "Human Impact":
            create_human_impact_dict_from_options(decision_point["options"])
            break


def create_human_impact_dict_from_options(options) -> None:
    for option in options:
        human_impact_value = option["label"]
        create_human_impact_dict_from_child_combinations(
            option["child_combinations"], human_impact_value
        )


def create_human_impact_dict_from_child_combinations(
    ...
    for child_combination in child_combinations:
        for child_combination_data in child_combination:
            if child_combination_data["child_label"] == "Situated Safety Impact":
                situated_safety_impacts = child_combination_data["child_option_labels"]
            if child_combination_data["child_label"] == "Mission Impact":
                mission_impacts = child_combination_data["child_option_labels"]
        for situated_safety_impact in situated_safety_impacts:
            for mission_impact in mission_impacts:
                human_impact_dict[(situated_safety_impact, mission_impact)] = human_impact_value


def create_ssvc_priority_dict(deployer_json) -> None:
