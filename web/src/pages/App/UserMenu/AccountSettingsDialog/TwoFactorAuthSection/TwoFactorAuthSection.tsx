...
type TwoFactorAuthSectionProps = {
  onShowSnackbar: (message: string, severity: AlertColor) => void;
};

export function TwoFactorAuthSection({ onShowSnackbar }: TwoFactorAuthSectionProps) {
  ...
  const handleDisableConfirm = () => {
    deletePhoneNumber()
      ...
      .catch((error: unknown) => {
        onShowSnackbar(authErrorToString(normalizeAuthErrorSource(error)) ?? "", "error");
      });
  };
