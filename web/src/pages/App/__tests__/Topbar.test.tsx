...
vi.mock("../PTeamCreateModal", () => ({
  PTeamCreateModal: ({ open }: { open: boolean }) =>
    open ? <div role="dialog">Create Team Modal</div> : null,
}));

vi.mock("../UserMenu/AccountSettings", () => ({
  AccountSettings: ({ accountSettingOpen }: { accountSettingOpen: boolean }) =>
...
describe("Topbar", () => {
  beforeEach(() => {
    ...
    vi.mocked(useAuth).mockReturnValue({ signOut: vi.fn() } as unknown as ReturnType<
      typeof useAuth
    >);
  ...
  it("resets client state and signs out from the user menu", async () => {
    ...
    vi.mocked(useAuth).mockReturnValue({ signOut } as unknown as ReturnType<typeof useAuth>);
    renderTopbar();
