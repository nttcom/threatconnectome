...
import type { PTeamCreateRequest } from "../../../types/types.gen";

type PTeamCreateModalProps = {
  open: boolean;
  onSetOpen: (open: boolean) => void;
  onCloseTeamSelector: () => void;
};

export function PTeamCreateModal({ open, onSetOpen, onCloseTeamSelector }: PTeamCreateModalProps) {
  const { t } = useTranslation("app", { keyPrefix: "PTeamCreateModal" });
  const navigate = useNavigate();
  const { enqueueSnackbar } = useSnackbar();

  const [pteamName, setPTeamName] = useState<string | null>(null);
  ...
  const handleCreate = async () => {
    if (!pteamName?.trim()) return;

    const data: PTeamCreateRequest = {
    ...
    await createPTeam({ body: data })
      ...
      .then(async (data) => {
        ...
        onSetOpen(false);
        // fix user.pteams before navigating, to avoid overwriting pteamId by pages/App.tsx.
