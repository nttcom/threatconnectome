...
function AuthRow(props) {
  ...
  const fixedSx = (isPseudo, enable) => ({
    color: editable && isPseudo === pseudoEdit ? "#1976d3" : enable ? "black" : "gray",
  });
...
export function ATeamAuthEditorMain(props) {
  const { ateamId, userId, userEmail, onClose, authInfo, uuidOthers, uuidMember, currentAuth } =
    props;
  ...
  const handleSave = async (targets) => {
    ...
    await updateATeamAuth({ ateamId, data })
      .unwrap()
...
export function ATeamAuthEditor(props) {
  const { ateamId, userId, userEmail, onClose } = props;
  ...
  } = useGetATeamAuthInfoQuery(undefined, { skip });
  const {
    data: ateamAuth,
    error: ateamAuthError,
    isLoading: ateamAuthIsLoading,
  } = useGetATeamAuthQuery(ateamId, { skip });

  if (skip) return <></>;
  if (userMeError) return <>{`Cannot get UserInfo: ${errorToString(userMeError)}`}</>;
  if (userMeIsLoading) return <>Now loading UserInfo...</>;
  if (authInfoError) return <>{`Cannot get ATeam: ${errorToString(authInfoError)}`}</>;
  if (authInfoIsLoading) return <>Now loading Auth Info...</>;
  if (ateamAuthError) return <>{`Cannot get Authorities: ${errorToString(ateamAuthError)}`}</>;
  if (ateamAuthIsLoading) return <>Now loading Authorities...</>;
